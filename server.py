...
mcp = FastMCP(
    "intervals-icu",
    instructions=(
        "Training data from intervals.icu for a single athlete. Use training_summary for the overview, pace_at_hr_trend to check "
        "whether aerobic fitness is drifting, and wellness for recovery signals. "
        "All distances are km, durations minutes, paces min/km."
    ),
    host="0.0.0.0",
    port=PORT,
    stateless_http=True,
...
async def training_summary(weeks: int = 6) -> dict:
    ...
    buckets: dict[tuple[date, str], dict] = defaultdict(
        lambda: {"sessions": 0, "km": 0.0, "minutes": 0.0, "load": 0.0}
    )
    ...
    return {
        ...
        "note": "km and hours are per sport per ISO week (weeks start Monday).",
    }
...
async def list_activities(
    ...
    Args:
        ...
        activity_type: Optional filter, e.g. "Run", "Ride", "Swim". Case-insensitive.
    ...
    if activity_type:
        ...
        activities = [a for a in activities if str(a.get("type", "")).lower() == wanted]
        ...
        rows.append(
            {
                ...
                "pace_min_km": _pace(act.get("distance"), act.get("moving_time")),
                "avg_hr": _num(act.get("average_heartrate")),
                "max_hr": _num(act.get("max_heartrate")),
                "avg_watts": _num(act.get("icu_weighted_avg_watts")),
...
async def activity_detail(activity_id: str) -> dict:
    ...
    d = _act_date(act) if isinstance(act, dict) else None
            ...
            intervals.append(
                {
                    ...
                    "seconds": _num(it.get("moving_time")) or _num(it.get("elapsed_time")),
                    "km": round((_num(it.get("distance")) or 0) / 1000, 3) or None,
                    "pace_min_km": _pace(it.get("distance"), it.get("moving_time")),
...
async def pace_at_hr_trend(
    ...
    Args:
        ...
        activity_type: Sport to analyse, e.g. "Run" or "Ride".
    ...
    for act in activities:
        if str(act.get("type", "")).lower() != wanted:
            continue
        d = _act_date(act)
    ...
    if len(per_week) >= 4:
        ...
        edges = [min(per_week), max(per_week)]
        for wk in edges:
            if len(per_week[wk]) * 2 < typical:
                dropped_partial.append(wk.isoformat())
                del per_week[wk]

    series = []
    for wk in sorted(per_week):
        rows = per_week[wk]
        mean_s_km = statistics.mean(r["s_per_km"] for r in rows)
        series.append(
            {
                "week_start": wk.isoformat(),
                "sessions": len(rows),
                "km": round(sum(r["km"] for r in rows), 1),
                "mean_hr": round(statistics.mean(r["hr"] for r in rows), 1),
                "mean_pace_min_km": f"{int(mean_s_km // 60)}:{int(mean_s_km % 60):02d}",
                "efficiency_index": round(statistics.mean(r["ef"] for r in rows), 3),
            }
        )
    ...
    if len(series) >= 4:
        half = len(series) // 2
        early = statistics.mean(s["efficiency_index"] for s in series[:half])
        late = statistics.mean(s["efficiency_index"] for s in series[half:])
    ...
    if len(series) >= 2:
        ...
        if spread > 5 and min_hr is None and max_hr is None:
            confound = (
